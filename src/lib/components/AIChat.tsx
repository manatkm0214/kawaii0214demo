...
export default function AIChat({ transactions, budgets, currentMonth }: Props) {
  ...
  const selectedProviderMeta = AI_PROVIDERS.find((item) => item.key === provider) ?? AI_PROVIDERS[0];
  const requestedProviderMeta = lastRequestedProvider
    ? (AI_PROVIDERS.find((item) => item.key === lastRequestedProvider) ?? selectedProviderMeta)
    : selectedProviderMeta;
  const responseProviderMeta = responseProvider
    ? (AI_PROVIDERS.find((item) => item.key === responseProvider) ?? selectedProviderMeta)
    : null;
  ...
  async function sendMessage(text: string) {
    ...
    setInput("");
    setLoading(true);
    setLastRequestedProvider(provider);
    ...
    try {
      ...
      const payload = (await response.json()) as { reply?: string; error?: string; provider?: AIProvider };
      if (!response.ok || !payload.reply) {
        throw new Error(payload.error || t("AIチャットの応答を取得できませんでした。", "Failed to get an AI chat reply."));
      }

      setResponseProvider(payload.provider ?? provider);
    ...
    <section className="metric-shell rounded-[28px] border border-slate-700 shadow-sm">
      <div className="border-b border-slate-700 px-4 py-4">
        ...
        <p className="mt-2 text-xs text-slate-300">
          {loading
            ? t(`選択中のAI: ${selectedProviderMeta.label} で応答中`, `Responding with ${selectedProviderMeta.label}`)
            : responseProviderMeta && responseProviderMeta.key !== requestedProviderMeta.key
              ? t(
                  `選択したAI: ${requestedProviderMeta.label} / 実際の応答: ${responseProviderMeta.label}`,
                  `Selected: ${requestedProviderMeta.label} / Responded: ${responseProviderMeta.label}`,
                )
              : responseProviderMeta
                ? t(`応答したAI: ${responseProviderMeta.label}`, `Responded: ${responseProviderMeta.label}`)
                : t(`選択中のAI: ${selectedProviderMeta.label}`, `Selected: ${selectedProviderMeta.label}`)}
        </p>
        ...
        <div className="flex gap-2 overflow-x-auto border-t border-slate-700 px-4 py-2">
          {quickPrompts.slice(0, 4).map((prompt) => (
            <button
              ...
              onClick={() => void sendMessage(prompt)}
              disabled={loading}
              className="shrink-0 rounded-full border border-slate-600 bg-slate-800 px-3 py-1 text-xs text-slate-300 transition hover:border-cyan-400 hover:text-cyan-200 disabled:opacity-40"
            >
              {prompt}
            </button>
          ))}
