...
export default function BudgetTradeoffPanel() {
  ...
  return (
    <div className="board-card border shadow-sm rounded-[28px] p-4 bg-white">
          ...
          {rules.map((rule) => (
            <div
              ...
              className={`flex items-center gap-2 rounded-2xl border px-3 py-2 ${
                rule.is_active ? "border-cyan-300 bg-cyan-50" : "border-slate-300 bg-slate-100 opacity-60"
              }`}
            >
              ...
              <button
                ...
                className="shrink-0 rounded-full border border-slate-300 px-2 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-100"
              >
                ✕
              </button>
            </div>
      ...
      {showForm ? (
        <div className="mt-3 space-y-3 rounded-2xl border border-slate-300 p-3">
          <div>
            ...
            <div className="grid gap-2 sm:grid-cols-[1fr_auto_1fr]">
              <label className="block">
                <span className="text-xs font-semibold text-black">{t("このカテゴリで使ったら", "When spending in")}</span>
                <select
                  value={trigger}
              ...
              <div className="flex items-end pb-2 text-black font-bold">→</div>
              <label className="block">
                <span className="text-xs font-semibold text-black">{t("この予算を削減", "Reduce this budget")}</span>
                <select
                  value={target}
          ...
          <label className="block">
            ...
            />
            <div className="flex justify-between text-xs font-semibold text-black mt-0.5">
